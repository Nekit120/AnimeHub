...
class ProfilePage extends BaseView<ProfileViewModel> {
   ProfilePage({super.key,required ProfileRepository profileRepository}): super(
    vmFactory: (context) => ProfileViewModel(context, profileRepository: profileRepository)
  );
    ...
    return AppBar(
      ...
      actions: [
        IconButton(
            ...
            onPressed: () {
              AutoRouter.of(vm.context).push(EditingProfileRoute(profileRepository: AppContainer().repositoryScope.profileRepository, userModel: vm.currentUser.value! ));
            }),
      ],
    );
  }
  ...
  Widget _personalProfileBody({required ProfileViewModel vm}) {
   return vm.currentUser.observer((context, value) =>
    value == null ? const Center(child: CircularProgressIndicator())
        : Padding(
      ...
      child: Column(
        ...
        children: [
          Center(
            child: SizedBox(
              ...
              child: ClipRRect(
                ...
                child:  value.profileImageUrl ==null ? Image.network(
                    "https://www.wild-pro.ru/wp-content/uploads/2023/04/no-profile-min.png")
                : Image.network(value.profileImageUrl!, fit: BoxFit.cover,)
                ),
              ),
            ),
          Center(
          ...
          const SizedBox(height: 16),
          Text("E-mail", style: Theme.of(vm.context).textTheme.labelMedium!),
          Text(value.email,
              style: Theme.of(vm.context).textTheme.bodyLarge!),
          const SizedBox(height: 16),
          Text("Телефон", style: Theme.of(vm.context).textTheme.labelMedium!),
